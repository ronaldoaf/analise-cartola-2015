...
probs= [ getProbVitoriaEquipe(t, rod) for t in times]


#print getPontuacao('73635',rod)
...
for jog in jogadores2[rod-1]:
    if jog['posicao']=='atacante' and jog['mando']==True:
        #ids+=[ jog['jog_id'] ]
        #medias+=[ jog['media_ant'] ]
        #probs+=[ getProbVitoriaCod(jog['jog_id'], rod) ]
        #FDs+=[ dif(jog,'FD')/jog['num_jogos'] ]
        print jog['jog_id'], \
              '"'+getNomeJogadorPorId(jog['jog_id'])+'"', \
              jog['media_ant'],\
              getProbVitoriaCod(jog['jog_id'], rod), \
              dif(jog,'FD')/jog['num_jogos']
...
for ii in range(4,38):
    ...
    for posicao in ['GOL', 'LAT', 'ZAG', 'MEI', 'ATA', 'TEC']:
        ...
        for reg in sorted(freq_pos, key=lambda x: x[1], reverse=True)[:k]:
            print getNomeJogadorPorId(reg[0]), str(100*round(reg[1],4))+'%', int(getProp(reg[0],rodada,'mando' ))
